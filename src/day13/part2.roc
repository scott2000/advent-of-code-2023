...
findPossibleReflectionPoints : (List Nat, List Nat), List Tile -> (List Nat, List Nat)
findPossibleReflectionPoints = \(currentPossible, withSmudge), tiles ->
    possibleToSmudge =
        currentPossible
        |> List.keepIf \index -> isReflectionPointButSmudge tiles index
    ...
    (
        stayPossible,
        staySmudge |> List.concat possibleToSmudge,
    )
...
isReflectionPointButSmudge : List Tile, Nat -> Bool
isReflectionPointButSmudge = \tiles, index ->
    { before, others } = List.split tiles index
    size = Num.min (List.len before) (List.len others)
    List.map2 (List.takeLast before size) (List.reverse (List.takeFirst others size)) \a, b ->
