...
solveProblem = \grid ->
    northSouthLasers =
        ...
        |> List.mapWithIndex \_, index -> [
            { row: 0, col: Num.toI32 index, dir: South },
            { row: Num.toI32 (List.len grid) - 1, col: Num.toI32 index, dir: North },
        ]
        |> List.join
    ...
    westEastLasers =
        ...
        |> List.mapWithIndex \row, index -> [
            { row: Num.toI32 index, col: 0, dir: East },
            { row: Num.toI32 index, col: Num.toI32 (List.len row) - 1, dir: West },
        ]
        |> List.join

    List.concat northSouthLasers westEastLasers
    |> List.map \laser -> energyGivenLaser laser grid
    |> List.max
    |> Result.withDefault 0

energyGivenLaser : Laser, Grid -> Nat
energyGivenLaser = \laser, grid ->
    initialState = {
        seen: Set.single laser,
        active: Set.single laser,
    }
