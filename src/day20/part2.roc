...
solveProblem = \specs ->
    ...
    mods
    |> Dict.values
    |> List.findFirst \mod -> List.contains mod.outputs "rx"
    |> Result.map \mod ->
        mod.inputs
        |> List.map \input -> findCycleLength mods input
        |> List.walk 1 lcm
...
findCycleLength : Dict Str Module, Str -> Nat
findCycleLength = \mods, name ->
    findCycleLengthHelper mods name 0

findCycleLengthHelper : Dict Str Module, Str, Nat -> Nat
findCycleLengthHelper = \mods, name, count ->
    (newMods, newPulses) = handleAllPulses mods [signal "button" "broadcaster" Low]

    if List.any newPulses \{ dest, pulse } -> dest == name && pulse == Low then
        count + 1
    else
        findCycleLengthHelper newMods name (count + 1)
