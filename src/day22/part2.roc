...
solveProblem = \bricks ->
    ...
    |> sortBricks
    |> buildTower
    |> findAllRequiredBricks
    |> Dict.walk (Dict.empty {}) \current, _, allRequired ->
        Set.walk allRequired current \inner, required ->
            Dict.update inner required \entry ->
                when entry is
...
findAllRequiredBricks : Tower -> Dict Nat (Set Nat)
findAllRequiredBricks = \tower ->
    Dict.walk tower.bricks (Dict.empty {}) \required, brick, _ ->
        findRequiredBricks tower required brick

findRequiredBricks : Tower, Dict Nat (Set Nat), Nat -> Dict Nat (Set Nat)
findRequiredBricks = \tower, required, brick ->
    sets =
        Dict.get tower.below brick
        |> Result.withDefault []
        |> List.map \child ->
            when Dict.get required child is
                Ok found ->
                    Set.insert found child

                Err _ ->
                    crash "Not found: \(Num.toStr child)"

    intersection =
        when sets is
            [] -> Set.empty {}
            [head, .. as tail] ->
                List.walk tail head Set.intersection

    Dict.insert required brick intersection
